...
exports.default = (db) => {
    ...
    router.post('/', [
        ...
        (0, express_validator_1.body)('items.*.productId').isInt(),
        (0, express_validator_1.body)('items.*.quantity').isInt({ min: 1 }),
        (0, express_validator_1.body)('shippingAddress').isObject(),
        (0, express_validator_1.body)('billingAddress').isObject(),
        (0, express_validator_1.body)('paymentMethod').isIn(['card', 'paypal', 'bank_transfer']),
        (0, express_validator_1.body)('shippingMethod').isString()
        ...
        for (const item of items) {
            const product = await db.get('SELECT id, name, price, stock_quantity, sku FROM products WHERE id = ? AND is_active = 1', [item.productId]);
            if (!product) {
  ...
  `, [
    ...
    `, [
            ...
            await db.run('UPDATE products SET stock_quantity = stock_quantity - ? WHERE id = ?', [item.quantity, item.productId]);
        }
        // Clear user's cart
        ...
        res.status(201).json({
            ...
            data: {
                order: {
                    ...
                    items: orderItems_query.map((item) => ({
  ...
  `;
    ...
    router.get('/:orderId', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    router.get('/track/:orderNumber', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    router.patch('/:orderId/status', (0, errorHandler_1.asyncHandler)(async (req, res) => {
        const { orderId } = req.params;
