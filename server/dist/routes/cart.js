...
exports.default = (db) => {
  ...
  `;
    ...
    router.post('/', [
        ...
        (0, express_validator_1.body)('quantity').optional().isInt({ min: 1, max: 10 })
        ...
        const product = await db.get('SELECT id, name, price, stock_quantity FROM products WHERE id = ? AND is_active = 1', [productId]);
        if (!product) {
        ...
        if (existingCartItem) {
            ...
            const result = await db.run('INSERT INTO cart_items (user_id, product_id, quantity) VALUES (?, ?, ?)', [userId, productId, quantity]);
            res.status(201).json({
                success: true,
    ...
    router.put('/:itemId', [
        (0, express_validator_1.body)('quantity').isInt({ min: 1, max: 10 })
    ], (0, errorHandler_1.asyncHandler)(async (req, res) => {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
            return res.status(400).json({ errors: errors.array() });
        ...
        const cartItem = await db.get('SELECT ci.*, p.stock_quantity, p.price FROM cart_items ci JOIN products p ON ci.product_id = p.id WHERE ci.id = ? AND ci.user_id = ?', [itemId, userId]);
        if (!cartItem) {
        ...
        await db.run('UPDATE cart_items SET quantity = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND user_id = ?', [quantity, itemId, userId]);
    ...
    router.delete('/:itemId', (0, errorHandler_1.asyncHandler)(async (req, res) => {
        const userId = req.user.id;
        const { itemId } = req.params;
        const result = await db.run('DELETE FROM cart_items WHERE id = ? AND user_id = ?', [itemId, userId]);
        if (result.changes === 0) {
            return res.status(404).json({ error: 'Cart item not found' });
        }
        res.json({
    ...
    router.post('/move-to-wishlist/:itemId', (0, errorHandler_1.asyncHandler)(async (req, res) => {
        const userId = req.user.id;
        const { itemId } = req.params;
        // Get cart item
        const cartItem = await db.get('SELECT * FROM cart_items WHERE id = ? AND user_id = ?', [itemId, userId]);
        if (!cartItem) {
            return res.status(404).json({ error: 'Cart item not found' });
        }
        // Check if product already in wishlist
        const existingWishlistItem = await db.get('SELECT id FROM wishlist WHERE user_id = ? AND product_id = ?', [userId, cartItem.product_id]);
        if (existingWishlistItem) {
            return res.status(400).json({ error: 'Item already in wishlist' });
        }
        // Add to wishlist
        await db.run('INSERT INTO wishlist (user_id, product_id) VALUES (?, ?)', [userId, cartItem.product_id]);
        // Remove from cart
